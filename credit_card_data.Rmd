...
percentages <- function(variable_x) {
  data %>%
  group_by(Attrition_Flag, unlist(variable_x)) %>%
  summarise(n = n()) %>%
  mutate(prop = n/sum(n),
...
percentages(data$Gender)
percentages(data$Education_Level)
percentages(data$Marital_Status)
percentages(data$Income_Category)
percentages(data$Card_Category)
