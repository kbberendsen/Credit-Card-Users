...
plot <- function(variable_x, variable_name){
  ...
  if(variable_x %in% numeric_variables){
   ...
   histogram <-  ggplot(data, aes(x = unlist(variable_x), fill = Attrition_Flag)) +
      geom_histogram(binwidth = 1, stat = "count", position = position_dodge(preserve = "single"), alpha = 0.5) +
      labs(title =  substitute(paste("Difference between Customer Types in terms of ", variable_name)),
           x = variable_name,
           y = "Count",
           fill = "Customer Type") +
    ...
    percentage_plot <- ggplot(data, aes(x = Attrition_Flag, fill = unlist(variable_x))) +
